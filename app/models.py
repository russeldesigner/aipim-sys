...
class Produto(db.Model):
    ...
    preco_kg = db.Column(Numeric(10, 2), default=0)
...
class Colaborador(db.Model):
    ...
    diaria  = db.Column(Numeric(10, 2), default=0)
    ativo   = db.Column(db.Boolean, default=True)
...
class Lancamento(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('data', name='uq_lancamento_data'),)
    ...
    peso_medio       = db.Column(Numeric(8, 2), default=0)
    mo_roca          = db.Column(Numeric(10, 2), default=0)
    hrs_trator1      = db.Column(Numeric(6, 2), default=0)
    hrs_trator2      = db.Column(Numeric(6, 2), default=0)

    # ── Logística ──
    km_ini_strada    = db.Column(Numeric(10, 1), default=0)
    km_fim_strada    = db.Column(Numeric(10, 1), default=0)
    km_ini_ducato    = db.Column(Numeric(10, 1), default=0)
    km_fim_ducato    = db.Column(Numeric(10, 1), default=0)
    motorista1       = db.Column(db.String(60))
    motorista2       = db.Column(db.String(60))
    motorista3       = db.Column(db.String(60))
    pedagio          = db.Column(Numeric(10, 2), default=0)
    estacionamento   = db.Column(Numeric(10, 2), default=0)

    # ── Custos gerais ──
    mo_galp   = db.Column(Numeric(10, 2), default=0)
    mo_desc   = db.Column(Numeric(10, 2), default=0)
    mo_coz    = db.Column(Numeric(10, 2), default=0)
    mo_obra   = db.Column(Numeric(10, 2), default=0)
    cafe      = db.Column(Numeric(10, 2), default=0)
    imposto   = db.Column(Numeric(10, 2), default=0)

    # ── Observações ──
    obs = db.Column(db.Text)

    movimentos = db.relationship(
        'MovimentoEstoque', backref='lancamento',
        lazy='select',   # carrega tudo de uma vez (evita N+1 vs lazy='dynamic')
        cascade='all, delete-orphan',
    )

    @property
    def km_strada(self):
        return max(0, float(self.km_fim_strada or 0) - float(self.km_ini_strada or 0))

    @property
    def km_ducato(self):
        return max(0, float(self.km_fim_ducato or 0) - float(self.km_ini_ducato or 0))

    @property
    def custo_total(self):
        campos = [self.mo_roca, self.mo_galp, self.mo_desc, self.mo_coz,
                  self.mo_obra, self.cafe, self.imposto, self.pedagio, self.estacionamento]
        return float(sum(v or 0 for v in campos))

    @property
    def venda_bruta(self):
        # movimentos já carregados (lazy='select') — sem N+1
        return float(sum(m.venda_total for m in self.movimentos if m.tipo == 'saida'))
...
class MovimentoEstoque(db.Model):
    ...
    quantidade    = db.Column(Numeric(10, 3), default=0)   # kg com 3 casas
    preco_unit    = db.Column(Numeric(10, 2), default=0)   # R$/kg

    @property
    def venda_total(self):
        if self.tipo == 'saida':
            return float(self.quantidade or 0) * float(self.preco_unit or 0)
        return 0.0
